...
export type DeepSearchRunStatus = "pending" | "running" | "searching" | "synthesizing" | "completed" | "failed";
export type DeepSearchTimelineEntryStatus = "pending" | "running" | "completed" | "failed";
...
export function updateDeepSearchRun(runId: string, update: Partial<Pick<DeepSearchRunRecord, "status" | "currentPhase" | "progressPct" | "message" | "hitsCount" | "completedAt" | "error">>): void {
  const run = ensureRun(runId);
  Object.assign(run, update);
}

export function addDeepSearchTimelineEntry(runId: string, entry: Omit<DeepSearchTimelineEntry, "id"> & { id?: string }): string {
  const run = ensureRun(runId);
...
export function updateDeepSearchTimelineEntry(runId: string, entryId: string, update: Partial<Omit<DeepSearchTimelineEntry, "id">>): void {
  const run = ensureRun(runId);
...
export function addSubQueryTimelineEntries(runId: string, subQueries: SubQuery[]): void {
  const run = ensureRun(runId);
...
export function getDeepSearchRun(runId: string): DeepSearchRunRecord | undefined {
