...
export async function handleTodoList(
  ...
  if (todos.length === 0) return formatChatResult("No pending tasks!");
  const out = todos
    .map(
      (t) =>
    ...
    .join("\n");
  return formatChatResult(`[TODO] Pending tasks:\n${out}`);
}
...
export async function handleTodoAdd(
  ...
  return formatChatResult(`✅ Added: ${todo.title}`);
}
...
export async function handlePlan(
  ...
  return formatChatResult(
    `[TODO] Created ${todos.length} tasks for "${goal}":\n${list}`,
  );
}
...
export async function handleTodoRemove(
  ...
  if (!id) return formatChatResult("Please provide a task ID to remove.");
  const success = removeTodo(id);
  return formatChatResult(
    success ? `✅ Removed task: ${id}` : `❌ Task not found: ${id}`,
  );
}
...
export async function handleTodoRemoveAll(
  ...
  removeAllTodos();
  return formatChatResult("✅ All tasks removed.");
}
